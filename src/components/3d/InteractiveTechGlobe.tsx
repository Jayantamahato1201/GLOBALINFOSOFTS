...
export const InteractiveTechGlobe: React.FC<{ className?: string }> = ({ className = '' }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const height = Math.max(container.clientHeight || 380, 1);
      ...
      const handleResize = () => {
        try {
          ...
          const h = Math.max(container.clientHeight || 380, 1);
          camera.aspect = w / h;
      ...
      console.warn('InteractiveTechGlobe WebGL fallback activated:', e);
    }

    return () => {
      try {
        if (onMouseDown && container) {
