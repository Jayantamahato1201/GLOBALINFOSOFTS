  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          onClick={onClose}
          className="absolute top-3 right-3 z-20 w-8 h-8 rounded-full flex items-center justify-center bg-slate-100/80 dark:bg-white/10 hover:bg-slate-200 dark:hover:bg-white/20 border border-slate-200/80 dark:border-white/10 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-all shadow-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/50"
        ...
        <div className="p-4 sm:p-5 md:p-6 overflow-y-auto max-h-full">
            ...
            <div className="md:col-span-6 flex flex-col justify-between space-y-2.5">
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-1.5 flex-wrap pr-8">
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-mono font-bold uppercase tracking-wider bg-cyan-500/15 text-cyan-700 dark:text-cyan-300 border border-cyan-500/30">
                    {data.category}
                  </span>
                  {data.duration && (
                ...
                <h2
                  ...
                  className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white font-['Outfit'] leading-snug"
                >
                  {data.title}
                </h2>

                {/* Short Description (2-3 lines) */}
                <p className="text-slate-600 dark:text-slate-300 text-xs sm:text-[13px] leading-relaxed">
                  {data.description}
                </p>
                ...
                {data.technologies && data.technologies.length > 0 && (
                  <div className="pt-1 space-y-1">
                    <div className="text-[9px] font-mono font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                      Technologies & Tools
                    </div>
                    <div className="flex flex-wrap gap-1">
                      {data.technologies.slice(0, 6).map((tech, idx) => (
                        <span
                          key={idx}
                          className="px-2 py-0.5 rounded-md bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-200 text-[11px] font-mono font-medium"
                        >
                          {tech}
                        </span>
                      ))}
              ...
              {/* Action Buttons */}
              <div className="pt-2.5 border-t border-slate-200 dark:border-white/10 flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                {data.primaryActionLabel && (
                  <button
                    ...
                    className="flex-1 py-2 px-3.5 rounded-lg btn-primary text-white font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-1.5 shadow-md transition-transform active:scale-95"
                  >
                ...
                {data.secondaryActionLabel && (
                  <button
                    ...
                    className="py-2 px-3.5 rounded-lg glass-card text-slate-700 dark:text-slate-200 hover:text-cyan-700 dark:hover:text-cyan-300 border-slate-200 dark:border-white/10 hover:border-cyan-500/40 font-semibold text-xs transition-all flex items-center justify-center gap-1"
                  >
